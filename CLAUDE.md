...
│   └── docker-publish.yml              # Multi-arch GHCR publish on push to main + tags '*.*.*'
src/proxmox_mcp/
├── __init__.py
...
- **`.github/workflows/docker-publish.yml`** — builds and pushes multi-arch (`amd64` + `arm64`) image to `ghcr.io/akmalovaa/proxmox-mcp` on:
  ...
  - push of semver tag `X.Y.Z` (no `v` prefix) → tags `:X.Y.Z`, `:X.Y`, `:sha-<short>`
...
To cut a release: `git tag 1.2.3 && git push --tags`. `docker-publish.yml` will build and push `:1.2.3` and `:1.2`. `:latest` only moves on push to `main`, not on tag pushes.
